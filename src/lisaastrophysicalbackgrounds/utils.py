"""Utility functions for processing and plotting astrophysical background data."""
...
def _where(condition: Any, x: Any, y: Any) -> jax.Array:
    """Type-safe wrapper for jnp.where to satisfy strict type checkers like Pylance."""
    return cast(jax.Array, jnp.where(condition, x, y))


def get_GWB_plot(
    frequencies: jax.Array, omega_f: jax.Array, var_f: jax.Array, save_path: Path
):
    """Generate and save log-log plots for Gravitational Wave Background strength."""
    plt.loglog(frequencies, omega_f, label="Expectation value")
    plt.fill_between(
        frequencies,
        omega_f - jnp.sqrt(var_f),
        omega_f + jnp.sqrt(var_f),
        alpha=0.35,
    ...
    plt.fill_between(
        frequencies,
        omega_f - 3.0 * jnp.sqrt(var_f),
        omega_f + 3.0 * jnp.sqrt(var_f),
        alpha=0.15,
        label="3-sigma interval",
        color="tab:blue",
    )
    plt.ylabel(r"GWB strength $\Omega_{\rm{GW}}(f)$")
    plt.xlabel(r"Frequency $f$")
    plt.legend()
    plt.xlim(left=float(frequencies.min()), right=float(frequencies.max()))
    plt.ylim(bottom=1e-14, top=2e-9)
    plt.savefig(save_path, dpi=150, bbox_inches="tight")
    plt.close()


# * ================= Helper functions to import BinCodex format =================
"""These functions are directly taken from the Synthetic UCB Catalogs project.

Project source: (github.com/Synthetic-UCB-Catalogs). Therefore, all credits for
writing these functions go to the members of that project. These functions are merely
copied from the 'analysis-scripts' sub-repository to make this package as light weight
as possible. At the time of writing this part of the code (June 2026), there was no
pip-installable version of some package that contained the relevant code below.
"""
...
def load_T0_data(ifilepath, code=None, **kwargs):
    ...
    **kwargs
        metallicty : `float`
            metallicity of the data if code=='SEVN'; this is encoded in the path
    ...
    if code == "ComBinE_csv":
        ...
        # read in the data with the columns
        dat = pd.read_csv(
            ifilepath, sep="\\s+", skiprows=3, names=col_names, skip_blank_lines=True
        )
...
def select_evolutionary_states(d):
    """Select the WDMS and DWD populations at the formation of the WDs.
    ...
    WDMS1 = (
        d.loc[
            (d.type1.isin([21, 22, 23]) & (d.type2.isin([12, 121]))) & (d.semiMajor > 0)
        ]
    ...
    WDMS2 = (
        d.loc[
            (d.type2.isin([21, 22, 23]) & (d.type1.isin([12, 121]))) & (d.semiMajor > 0)
        ]
        .groupby("ID", as_index=False)
        .first()
    )
    ...
    DWD = (
        d.loc[
            (d.type1.isin([21, 22, 23]))
            & (d.type2.isin([21, 22, 23]))
            & (d.semiMajor > 0)
        ]
        .groupby("ID", as_index=False)
        .first()
    ...
    if len(DWD) == 0:
        WDMS1 = (
            d.loc[((d.type1 == 2) & (d.type2 == 121)) & (d.semiMajor > 0)]
            .groupby("ID", as_index=False)
            .first()
        )
        WDMS2 = (
            d.loc[((d.type2 == 2) & (d.type1 == 121)) & (d.semiMajor > 0)]
            .groupby("ID", as_index=False)
            .first()
        )
        ...
        WDMS = pd.concat([WDMS1, WDMS2])
        DWD = (
            d.loc[(d.type1 == 2) & (d.type2 == 2) & (d.semiMajor > 0)]
            .groupby("ID", as_index=False)
            .first()
        )
