...
def a_min(m1: jax.Array, m2: jax.Array) -> jax.Array:
    """Calculate minimal orbital separation between two white dwarfs.
...
def orbital_freq_kepler(m1: jax.Array, m2: jax.Array, a: jax.Array) -> jax.Array:
    """Calculate the Keplerian orbital frequency of a circular binary.

    Args:
        m1 (jax.Array): Mass of the object primary in solar masses.
        m2 (jax.Array): Mass of the secondary in solar masses.
        a (jax.Array): The orbital separation between the binary components in
            solar radii.
...
def K_factor(M_ch: jax.Array) -> jax.Array:
    """Calculate the GW frequency evolution factor of a circular binary system.
...
def tau_GW(f_start: jax.Array, f_end: jax.Array, K: jax.Array) -> jax.Array:
    """Calculate the characteristic timescale of a GW emission driven system.

    Computes the time required for a circular binary to evolve from an initial
    frequency f_start to an ending frequency f_end.

    Args:
        f_start (jax.Array): Considered starting frequency of the binary in Hz.
        f_end (jax.Array): Considered end frequency of the binary in Hz.
        K (jax.Array): GW frequency evolution factor, see function K_factor.
...
@jax.jit
def orbital_freq_from_time(
    nu_start: jax.Array, evolve_time: jax.Array, K: jax.Array
) -> jax.Array:
    """Calculate the orbital frequency of a circular system after time evolution.

    Args:
        nu_start (jax.Array): Starting orbital frequency of the binary in Hz.
        evolve_time (jax.Array): Time over which the binary evolves in Myr.
        K (jax.Array): GW frequency evolution factor, see function K_factor.

    Returns:
        jax.Array: The orbital frequency of the binary after evolving.
    """
    nu_start_83: jax.Array = (nu_start * nu_start * nu_start) / jnp.cbrt(nu_start)
    nu_end_inv_83: jax.Array = 1 / nu_start_83 - (8 * evolve_time * K) / (
        3 * TAU_IN_MYR_CONST
    )
...
def a_min_BHs(m1: jax.Array, m2: jax.Array) -> jax.Array:
    """Calculate minimal orbital separation between two black holes.
