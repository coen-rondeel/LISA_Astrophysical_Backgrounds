...
class PreprocessPopulation():
    ...
    def __init__(
        ...
        self.m2: jax.Array = jnp.asarray(m2) if m2 is not None else cast(jax.Array, None)
    ...
    def check_parameters(self) -> None:
        "This initializes the parameters if not imported in population function for strictly circular/quadrupolar WD binaries" 
        # TODO do this for other types?
        gwb_input_params = ["t0", "M_ch", "M_ch_pow", "K_factor", "nu0", "numax", "merger_time"]
    ...
    def simple_single_Z_import(self) -> None:
        """The simplest possible import of a galaxy population of BWDs with only one metallicity.
        """
    ...
    def simple_multi_Z_import(self) -> None:
        """The simplest possible import of a galaxy population of BWDs which has multiple metallicities.
        """
        population_df = pd.read_csv(self._config['population']['population_path'])
        population_df: DataFrame = population_df.apply(pd.to_numeric, errors='coerce')  # 'coerce' will turn invalid parsing into NaN
    ...
    def mock_BH_import(self) -> None:
        ...
        """
        population_df = pd.read_csv(self._config['population']['population_path'], usecols=['t0', 'a', 'm1', 'm2'])
    ...
    def get_M_ch(self) -> None:
        ...
        if self.m1 is None or self.m2 is None and M_ch_pow is None:
            raise ValueError("It appears that the mass related imports of the populations where unsuccessful. Please ensure that the population has mass attributes")
    ...
    def get_nu0(self) -> None:
        if self.m1 is None or self.m2 is None or self.a is None:
            raise ValueError("The population import should either provide m1, m2, and a, OR nu0.")
        else:
            self.nu0 = orbital_freq_kepler(self.m1, self.m2, self.a)
            
    def get_numax(self) -> None:
        if self.m1 is None or self.m2 is None or self.a is None:
            raise ValueError("The population import should either provide m1 and m2, OR numax.")
        else:
