...
RSUN_SI = 695700000 # nominal solar radius in meters (agrees with astropy and wolframalpha value)

KEPLER_CONST = (G_SI * MSUN_SI / (RSUN_SI**3))**(1/2) / (2.0 * PI)
GW_TIME_CONST = (96 / 5) * (2 * PI)**(8/3) * (G_SI * MSUN_SI)**(5/3) / (C_SI**5)
TAU_IN_MYR_CONST = 1 / (1e6 * YRSID_SI)
...
def a_min(m1: jax.Array, m2: jax.Array) -> jax.Array:
    ...
    ap_min = r1 * (0.6 + q_23 * jnp.log1p(1 / q_13)) / 0.49
    as_min = r2 * (0.6 + 1/q_23 * jnp.log1p(q_13)) / 0.49
...
def a_min_BHs(m1: jax.Array, m2: jax.Array) -> jax.Array:
    ...
    jax.Array
        The minimal orbital separation in solar radii.  
    """
    r_s1 = 2 * G_SI * m1 * MSUN_SI / C_SI**2 / RSUN_SI
    r_s2 = 2 * G_SI * m2 * MSUN_SI / C_SI**2 / RSUN_SI
...
def tau_GW(f_start: jax.Array, f_end: jax.Array, K: jax.Array) -> jax.Array:
    ...
    """
    f_start_3 = f_start * f_start * f_start
    f_start_83 = f_start_3 / jnp.cbrt(f_start)

    f_end_3 = f_end * f_end * f_end
    f_end_83 = f_end_3 / jnp.cbrt(f_end)
...
def orbital_freq_from_time(nu_start: jax.Array, evolve_time: jax.Array, K: jax.Array) -> jax.Array:
    ...
    """
    nu_start_83 = (nu_start * nu_start * nu_start) / jnp.cbrt(nu_start)
    nu_end_inv_83 = 1/nu_start_83 - (8 * evolve_time * K) / (3 * TAU_IN_MYR_CONST)
    nu_end_83 = 1 / nu_end_inv_83
    nu_end_13 = jnp.sqrt(jnp.sqrt(jnp.sqrt(nu_end_83)))
    nu_end = nu_end_13 * nu_end_13 * nu_end_13
    return nu_end